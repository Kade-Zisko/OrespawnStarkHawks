...
def load_kaggle() -> pd.DataFrame:
    """Load the Kaggle dataset containing CO2 readings."""
    path = os.path.join(BASE, "data", "SensorLog.csv")
    if not os.path.exists(path):
        return pd.DataFrame()
    df = pd.read_csv(path, sep=";")
    df["date"] = pd.to_datetime(df["Timestamp"], format="%y.%m.%d %H:%M:%S", utc=True)
    df["temperature"] = df["TempIn_1 (°C)"]
    df["humidity"] = df["HumIn_1 (%)"]
    df["co2"] = df["CO2_eq (ppm)"]
    df["hive_id"] = "kaggle_1"
    df["source"] = "kaggle"
    df["label"] = 0  # Assuming healthy since no labels provided
    df = df[["date", "hive_id", "temperature", "humidity", "co2", "label", "source"]]
    print(f"[Kaggle] {len(df):>8,} readings  hives={df['hive_id'].nunique()}")
    return _resample_hourly(df)
...
def load_all(verbose: bool = True) -> pd.DataFrame:
    """Load and merge all datasets into a unified hourly DataFrame."""
    print("=" * 60)
    print("Loading datasets...")
    print("=" * 60)
    parts = [load_urban(), load_mspb(), load_kaggle()]
