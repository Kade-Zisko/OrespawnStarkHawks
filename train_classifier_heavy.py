"""
Beehive Health Classifier — Heavy variant.

Improvements over train_classifier_v2.py
-----------------------------------------
- Extended feature set: 12h + 72h rolling windows, circadian encoding
  (hour_sin/cos), seasonal encoding (doy_sin/cos), temp×humidity ratio.
  41 features total (vs 16 in v2).
- Deeper/wider model: N → 128 → 64 → 32 → 16 → 1, BatchNorm + Dropout.
- Focal loss (α=0.25, γ=2) instead of BCE — handles class imbalance at the
  gradient level, complementing SMOTE's sample-level rebalancing.
- ReduceLROnPlateau scheduler (halves LR after 5 stagnant epochs).
- Patience extended to 15 epochs, max epochs 300.
- Optimal decision threshold derived per CV fold (maximises F1-stressed),
  averaged and saved as threshold.pkl for inference.
- Saves to models_heavy/.
"""
...
MODEL_DIR = os.path.join(BASE, "models_heavy")
...
# Per-hive z-score normalization is applied only to sensor-derived cols.
# Temporal encodings (sin/cos) are already in [-1, 1] and bypass it.
SENSOR_COLS = [
...
def optimal_threshold(y_true: np.ndarray, y_prob: np.ndarray) -> float:
    ...
    # thresholds has one fewer element than prec/rec
...
# ── 3. Per-hive z-score normalization (sensor cols only) ──────────────────────

print(f"Per-hive z-score normalization (baseline = first {BASELINE_DAYS} days)...")
hive_stats: dict = {}
...
report = {
    "model": (
        ...
        f"BatchNorm + Dropout={DROPOUT}  FocalLoss  ReduceLROnPlateau  early_stopping"
    ),
    "architecture": {
    ...
    "dataset": {
        ...
        "normalisation": (
            "per-hive z-score (first-21-day baseline, sensor cols only) + StandardScaler"
        ),
    },
...
print(
    f"\nSaved to models_heavy/:  hive_model.pkl  hive_model.onnx  scaler.pkl  "
    f"feature_cols.pkl  threshold.pkl  hive_stats.json  report.json"
)
print("Done.")
