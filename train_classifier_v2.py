"""
Beehive Health Classifier v2 — dropout + early stopping.

Changes vs train_classifier.py
-------------------------------
- HiveNet uses dropout=0.3 between hidden layers
- Training uses early stopping (patience=7) instead of fixed epochs
- CV folds serve as the validation signal for early stopping
- Final model uses a 10 % internal val split for early stopping
- Saves to models_v2/
"""
...
def make_loader(X: np.ndarray, y: np.ndarray) -> DataLoader:
    ...
    dataset = TensorDataset(torch.FloatTensor(X_res), torch.FloatTensor(y_res))
    return DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)


def train_model(X_tr: np.ndarray, y_tr: np.ndarray,
                X_val: np.ndarray, y_val: np.ndarray) -> tuple[HiveNet, int]:
    model     = HiveNet(X_tr.shape[1], dropout=DROPOUT)
...
# ── 6. 5-fold CV (val fold used for early stopping) ───────────────────────────
...
# ── 7. Train final model (10 % internal val for early stopping) ───────────────
...
report = {
    "model": "HiveNet MLP v2  14 → 32 → 16 → 1  dropout=0.3  early_stopping",
    "architecture": {"layers": [14, 32, 16, 1], "activation": "ReLU",
                     "output": "Sigmoid", "dropout": DROPOUT},
    "training": {"max_epochs": MAX_EPOCHS, "patience": PATIENCE, "lr": LR,
...
print(f"\nSaved: models_v2/hive_model.pkl  hive_model.onnx  scaler.pkl  "
